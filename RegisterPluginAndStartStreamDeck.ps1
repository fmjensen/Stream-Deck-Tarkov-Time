...
$pluginProjectFile = "$basePath\TarkovTime.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"TarkovTime`" project before calling this script.";
  exit 1;
}
...
Write-Host "Deployment complete. Restarting the Stream Deck desktop application... `n"
